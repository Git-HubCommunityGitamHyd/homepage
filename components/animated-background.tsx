...
export function AnimatedBackground() {
  ...
  useEffect(() => {
    const generateElements = () => {
      ...
      // Generate 50 floating elements for richer background
      for (let i = 0; i < 50; i++) {
        newElements.push({
          id: i,
          ...
          opacity: Math.random() * 0.25 + 0.05, // 0.05-0.80 (increased visibility)
        })
      }
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      {elements.map((element) => (
        ...
        >
          <element.Icon size={element.size} className="text-gray-400" style={{ opacity: element.opacity }} />
        </motion.div>
      ))}
      ...
      <motion.div
        className="absolute top-3/4 right-1/4 w-24 h-24 border border-gray-300 rounded-full"
        animate={{
          scale: [1.2, 1, 1.2],
      ...
      <motion.div
        className="absolute top-1/2 right-1/3 w-16 h-16 border border-gray-300 rounded-full"
        animate={{
          scale: [1, 1.5, 1],
          opacity: [0.2, 0.1, 0.2],
        }}
