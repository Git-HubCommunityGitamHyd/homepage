...
export function EventPopupCard({ event, index }: EventPopupCardProps) {
  const frontContent = (
    <Card className="h-full bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 hover:border-gray-300 transition-all duration-300 hover:shadow-lg">
  ...
  const backContent = (
    <div className="h-full flex flex-col min-h-[700px]">
      ...
      <div className="mb-8">
        ...
        <motion.div
          className="flex flex-wrap gap-6 text-base text-gray-600 mb-6"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <Badge variant="outline" className="border-black text-black px-6 py-3 text-base">
            {event.category}
          </Badge>
        </motion.div>
      </div>
      ...
      >
        <h4 className="text-2xl font-semibold mb-6 text-gray-800">Event Details</h4>
        <p className="text-gray-600 leading-relaxed text-xl">{event.description}</p>
      </motion.div>
      ...
      >
        <h4 className="text-2xl font-semibold mb-8 text-gray-800 flex items-center gap-4">
