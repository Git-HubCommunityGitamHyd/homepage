...
export function FloatingGitHubElements() {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      ...
      >
        <Github className="w-32 h-32 text-gray-100 dark:text-gh-elevated opacity-30" />
      </motion.div>
      ...
      >
        <Github className="w-20 h-20 text-gray-100 dark:text-gh-elevated opacity-20" />
      </motion.div>

      <motion.div
        className="absolute bottom-1/4 left-1/3"
        animate={{ x: [-10, 10, -10], y: [10, -10, 10], rotate: [0, -10, 10, 0] }}
        transition={{ duration: 10, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 4 }}
      >
        <Github className="w-24 h-24 text-gray-100 dark:text-gh-elevated opacity-25" />
      </motion.div>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <motion.div
          ...
          transition={{ duration: Math.random() * 10 + 8, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: Math.random() * 5 }}
        >
          <Github
      ...
      <svg className="absolute inset-0 w-full h-full opacity-5">
        ...
        <motion.path
          ...
          initial={{ pathLength: 0 }}
          animate={{ pathLength: 1 }}
          transition={{ duration: 10, repeat: Number.POSITIVE_INFINITY, repeatType: "reverse", ease: "easeInOut", delay: 2 }}
        />
      </svg>
    </div>
  )
